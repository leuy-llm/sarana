...
class TourController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('images')) {
            foreach ($request->file('images') as $index => $image) {
                ...
                $isPrimary = $index == 0; // Mark the first image as the primary
                // Store image in tour_images table
                TourImage::create([
                    'tour_id' => $tour->id,
                    'image' => $path,
    ...
    //         'name' => 'required|string|max:255',
    //         'description' => 'required',
    //         'image' => 'nullable|image|mimes:jpeg,png,jpg,gif', // Allow nullable for optional update
    //         'price' => 'required|numeric',
    //         'duration' => 'required|string',
    //         'location' => 'required|string',
    //         'is_featured' => 'sometimes|boolean', // Changed to 'sometimes' for unchecked case
